...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        if self.mode == 'supervised':
            ...
            tbar = tqdm(range(len(self.supervised_loader)), ncols=150)
        else:
            dataloader = iter(zip(cycle(self.supervised_loader), self.unsupervised_loader))
            tbar = tqdm(range(len(self.unsupervised_loader)), ncols=150)
        ...
        for batch_idx in tbar:
            ...
            if self.mode == 'supervised':
                tbar.set_description('T ({}) | Ls {:.5f} Lu {:.5f} Lw {:.5f} PW {:.5f} IoU(change-l) {:.5f}|'.format(
                epoch, self.loss_sup.average, self.loss_unsup.average, self.loss_weakly.average,
                self.pair_wise.average, self.class_iou_l[1]))
            else:
                tbar.set_description('T ({}) | Ls {:.5f} Lu {:.5f} Lw {:.5f} PW {:.5f} IoU(change-l) {:.5f} IoU(change-ul) {:.5f}|'.format(
                epoch, self.loss_sup.average, self.loss_unsup.average, self.loss_weakly.average,
